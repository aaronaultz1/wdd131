...
// SIMILAR TO HOMEWORK BELOW

// add an event listener to allow the search to take place
let hikeQuery = document.querySelector("#search").value;

let filteredHikes = hikes.filter(function(hike){
    return (
        //Checks the hike name of each hike
        hike.name.toLowerCase().includes(hikeQuery.toLowerCase()) || //or
        //Checks the description of each hike
        hike.description.toLowerCase().includes(hikeQuery.toLowerCase()) ||
        //Checks one tag at a time within each hike
        hike.tags.find((tag) => tag.toLowerCase().includes(hikeQuery.toLowerCase()))
    )
});

// To alphebatize the filtered list 
...
let sortedFilteredHikes = filteredHikes.sort(compareHikes);

//prints the alphebatized filtered list of hikes
console.log(sortedFilteredHikes);